...
class VietstockSitemapCrawler(BaseCrawler):
    """Fallback sitemap-based crawler. Used only when the search API is unavailable."""
    ...
    async def crawl(
        ...
        try:
            ...
            logger.warning("[VietstockSitemapCrawler] Failed to fetch sitemap: %s", exc)
            return []

        results: list[dict[str, Any]] = []
        seen_urls: set[str] = set()

        logger.info("[VietstockSitemapCrawler] Sitemap returned %d URLs", len(urls))
        ...
        for url, _ in article_urls:
            ...
            try:
                ...
                logger.warning("[VietstockSitemapCrawler] Skipping article %s: %s", url, exc)
                continue
