...
class CafeFSitemapCrawler(BaseCrawler):
    """Fallback crawler that reads Cafef sitemap and extracts article content."""
    ...
    async def crawl(
        ...
        try:
            ...
            logger.warning("[CafeFSitemapCrawler] Failed to fetch sitemap: %s", exc)
            return []
        ...
        for url, lastmod in urls_with_dates:
            ...
            try:
                ...
                if article and article.get("symbols"):
                    results.append(article)
                elif article:
                    logger.debug(
                        "[CafeFSitemapCrawler] Skipping (no valid symbols): %s", url
                    )
                await asyncio.sleep(REQUEST_DELAY_SECONDS)
            except Exception as exc:
                logger.warning(
                    "[CafeFSitemapCrawler] Skipping article %s: %s", url, exc
                )
                continue
    ...
    async def _fetch_article_urls(self) -> list[tuple[str, datetime | None]]:
        ...
        async with httpx.AsyncClient(timeout=TIMEOUT_SECONDS, headers=headers) as client:
            ...
            logger.info("[CafeFSitemapCrawler] Sitemap parsed %d URLs", len(urls))
            return urls
    ...
    def _parse_sitemap(xml_text: str) -> list[tuple[str, datetime | None]]:
        ...
        try:
            ...
            logger.warning("[CafeFSitemapCrawler] Sitemap XML parse error: %s", e)
            return []
                ...
                if lastmod_text:
                    try:
                        lastmod = datetime.fromisoformat(
                            lastmod_text.replace("+07:00", "+07:00").rstrip("Z")
                        )
    ...
    def _parse_article(self, html: str, url: str) -> dict[str, Any] | None:
        ...
        for tag in tag_section:
            ...
            if re.fullmatch(r"[A-Z]{3,4}", tag_text) and is_valid_ticker(tag_text):
                symbol_tags.append(tag_text)
        symbol_tags = list(dict.fromkeys(symbol_tags))

        if not title or not content:
            logger.debug(
                "[CafeFSitemapCrawler] Skipping article (no title or content): %s", url
            )
            return None
